...
public class APIGenerator
{
	...
	private void declareProxyMembers(PrintStream out, Class clazz) throws Exception
	{
		...
		out.format("\t\tbool __TryInvoke(jmethodID, bool* success, jobject*, jobjectArray);\n");
		for (Method method : clazz.getDeclaredMethods())
		{
...
*/
	...
	private void implementProxy(PrintStream out, Class clazz) throws Exception
	{
		...
		for (Class interfaze : clazz.getInterfaces())
			out.format("%s::__Proxy::operator %s() { return %s(static_cast<jobject>(__ProxyObject())); }\n", getSimpleName(clazz), getClassName(interfaze), getClassName(interfaze));
		out.format("bool %s::__Proxy::__TryInvoke(jmethodID methodID, bool* success, jobject* object, jobjectArray args) {\n", getSimpleName(clazz));
		...
		for (Method method : methods)
		{
			...
			if (returnType != void.class)
				out.format("\tif (methodIDs[%d] == methodID) { *object = jni::NewLocalRef(static_cast< %s >(%s(%s))); *success = true; return true; }\n",
					i++,
					getClassName(box(returnType)),
					getMethodName(method),
					getParametersFromJNIObjectArray(params));
			else
				out.format("\tif (methodIDs[%d] == methodID) { *object = NULL; %s(%s); *success = true; return true; }\n",
					i++,
					getMethodName(method),
					getParametersFromJNIObjectArray(params));
		}
